...
#pragma HLS INLINE
    ...
    const input_t zero = 0;
    const int EXTRA_INT_BITS = 2;
    const int EXTRA_FRAC_BITS = 4;
    typedef ap_ufixed<W1 + EXTRA_FRAC_BITS, I1 + EXTRA_INT_BITS, AP_RND_CONV, AP_SAT> wide_t;

 //   input_t clamped = (x < zero) ? zero : x;
   // wide_t wide_in = (wide_t)clamped;
 //   wide_t wide_out = hls::sqrt(wide_in);
  //  input_t result = wide_out;

input_t clamped = (x < zero) ? zero : x;  // clamping value
wide_t wide_in = (wide_t)clamped;  // cast to wide_t (ap_ufixed)
float wide_in_float = (float)wide_in;  // cast to float for sqrt

float wide_out_float = hls::sqrt(wide_in_float);


wide_t wide_out = (wide_t)wide_out_float;

// Final result in input_t (ap_ufixed)
input_t result = wide_out;
...
#pragma HLS INLINE
    ...
    input_t clamped = (x < zero) ? zero : x;
    wide_t wide_in = (wide_t)clamped;
    wide_t wide_out = hls::rsqrt(wide_in);
    input_t result = wide_out;
